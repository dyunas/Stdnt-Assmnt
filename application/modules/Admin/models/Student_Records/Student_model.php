	...
	public function edit_student_assessment($stud_id)
	{
		for ($i = 0; $i < count($this->input->post('f_name')); $i++)
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				$this->db->select('*');
				$this->db->from('tbl_stud_asmnt_info');
				$this->db->where('fee_name', $row->fee_name);
				...
				if ($sql->num_rows() > 0)
				{
					...
					$this->db->where('stud_id', $stud_id);
					$this->db->where('stud_course', $this->input->post('stud_course'));
					$this->db->where('stud_year', $this->input->post('stud_year'));
					$this->db->where('stud_sem', $this->input->post('stud_semester'));
					$this->db->where('fee_name', $srow->fee_name);
					$query_2 = $this->db->update('tbl_stud_asmnt_info', $assmnt_data);
				}
				else
				{
					$assmnt_data = array(
		...
		switch($this->input->post('stud_pymnt_schm'))
		{
			case 'CSHPYMNT':
				...
				if ($sql2->num_rows() > 0)
				{
					...
					$this->db->where('stud_year', $this->input->post('stud_year'));
					$this->db->where('stud_sem', $this->input->post('stud_semester'));
					$query_3 = $this->db->update('tbl_stud_pybles_csh', $pybles_data);
				}
				else
				{
					$pybles_data = array(
			...
			case 'INSTLMNT':
				...
				if ($sql2->num_rows() > 0)
				{
					...
					$this->db->where('stud_year', $this->input->post('stud_year'));
					$this->db->where('stud_sem', $this->input->post('stud_semester'));
					$query_3 = $this->db->update('tbl_stud_pybles_inst', $pybles_data);
				}
				else
				{
					$pybles_data = array(
			...
			case 'MNTHLY':
				for($i = 0; $i < count($this->input->post('pymnt_for[]')); $i++)
				{
					...
					$this->db->where('stud_year', $this->input->post('stud_year'));
					$this->db->where('stud_sem', $this->input->post('stud_semester'));
					$this->db->where('pymnt_for', $this->input->post('pymnt_for['.$i.']'));
					...
					if ($sql2->num_rows() > 0)
					{
						...
						$this->db->where('stud_sem', $this->input->post('stud_semester'));
						$this->db->where('pymnt_for', $this->input->post('pymnt_for['.$i.']'));
						$query_3 = $this->db->update('tbl_stud_pybles_mnth', $pybles_data);
					}
					else
					{
						$pybles_data = array(
		...
		$this->db->where('stud_year', $this->input->post('stud_year'));
		$this->db->where('stud_sem', $this->input->post('stud_semester'));
		$query = $this->db->update('tbl_stud_pymnt_schm', $schm_data);
		...
		if ($query)
		{
			...
			$this->db->where('stud_course', $this->input->post('stud_course'));
			$this->db->where('stud_year', $this->input->post('stud_year'));
			$this->db->where('semester', $this->input->post('stud_semester'));
			$query = $this->db->update('tbl_stud_units', $units_data);
			...
			if ($query)
			{
				...
				$this->db->where('stud_year', $this->input->post('stud_year'));
				$this->db->where('stud_sem', $this->input->post('stud_semester'));
				$query = $this->db->update('tbl_stud_discount_info', $disc_data);
